...
(function () {
        ...
        if (!closed) {
            try {
                Socket.send(JSON.stringify(establishRPC(
                    Date.now(), detail, gamestate,
                    (document.querySelector("body").className.includes("anon")),
                    document.querySelector("#me_username").innerText,
                    document.querySelector("#me_level").innerText,
                    ((document.querySelector("#me_leaguerank").src).replace("https://tetr.io/res/league-ranks/", "")).replace(".png", "").toUpperCase()
                )));
            } catch (e) { }
        }
        else {
    ...
    Socket.onclose = function (Error) {
        console.error("# [TETRIO-RPC] Disconnected: " + Error);
        if (Error.code != "1000") {
            console.log("[TETRIO-RPC] Bridge not running on port 6680.");
        }
    };

    Socket.onopen = function () {
        console.log("[TETRIO-RPC] Connected to bridge");
    };
...
function establishRPC(timestamp, detail, gamestate, anon, username, lvl, rank) {
    ...
    if (detail == "40 LINES") sik = "mode_40l";
    if (detail == "BLITZ") sik = "mode_blitz";
    if (detail == "ZEN") sik = "mode_zen";
    if (detail == "CUSTOM GAME") sik = "mode_custom";
    if (detail == "CUSTOM ROOM") sik = "mode_custom";
    if (detail == "QUICK PLAY") sik = "mode_quickplay";
    if (detail == "TETRA LEAGUE") sik = "mode_league";
